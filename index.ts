...
class Discord {
	static DeleteWebhook( url ) {
		return new Promise(function(resolve, reject) {
			request.delete( url, function optionalCallback(err, httpResponse, body) {
				try {
...
Discord.DeleteWebhook( Main.GetArgs()[0] ).then( response => {
	if (response) {
		console.log(chalk.hex('#d6af42')(Main.formatConsoleDate(new Date())) + chalk.green("Webhook successfully deleted !"))
	}else{
		console.log(chalk.hex('#d6af42')(Main.formatConsoleDate(new Date())) + chalk.red("Webhook does not exist ..."))
	}
})
